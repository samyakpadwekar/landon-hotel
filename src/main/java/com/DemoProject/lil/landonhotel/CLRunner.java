...
/*
 * @Component public class CLRunner implements CommandLineRunner {
 * 
 ...
 * 
 * public CLRunner(RoomRepository roomRepository, GuestRepository
 * guestRepository, ReservationRepository reservationRepository,
 * RoomReservationService roomReservationService) { this.roomRepository =
 * roomRepository; this.guestRepository = guestRepository;
 * this.reservationRepository = reservationRepository;
 * this.roomReservationService = roomReservationService; }
 * 
 * @Override public void run(String... args) throws Exception {
 * System.out.println("*** GUESTS ***"); List<Guest> guests =
 * this.guestRepository.findAll(); guests.forEach(System.out::println);
 * System.out.println("*** ROOMS ***"); List<Room> rooms =
 * this.roomRepository.findAll(); rooms.forEach(System.out::println);
 * System.out.println("*** RESERVATIONS ***"); List<Reservation> reservations =
 * this.reservationRepository.findAll();
 * reservations.forEach(System.out::println);
 * 
 * System.out.println("*** ROOM RESERVATIONS FOR DATE***");
 * List<RoomReservation> reservationsByDate =
 * this.roomReservationService.getRoomReservationsForDate("2023-08-28");
